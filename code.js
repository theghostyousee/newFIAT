(function (factory) {
    ...
    */
    ...
    gsapWithCSS.registerPlugin(ScrollTrigger);
    function fadeIn() {
      document.arrive('.fade-in', {
        existing: true
      }, function () {
        const node = this;
        const section = node.closest('.section');
        let startX = node.getAttribute('data-start-x');
        let startY = node.getAttribute('data-start-y');
        let endX = node.getAttribute('data-end-x');
        let endY = node.getAttribute('data-end-y');
        let delay = parseFloat(node.dataset.delay);
        let time = parseFloat(node.dataset.time);
        let alpha = parseFloat(node.dataset.alpha);
        let start = node.dataset.start;
        let selector = node.getAttribute('data-trigger');
        let trigger = node.closest(selector);
    ...
    window.scrollTo(0, 0); // Utils / Widgets
